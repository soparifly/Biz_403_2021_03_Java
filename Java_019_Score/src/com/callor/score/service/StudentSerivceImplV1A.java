package com.,                                                               c`allor.score.service;
...
public class StudentSerivceImplV1A implements StudentService {
	...
	/*
	 * new StudentServiceImplV1()생성자를 호출하여 객체를 생성하려고 하면 고정된 파일 이름(" "src/com/callor/score/student.txt")을
	 * 사용하여 파일을 읽을 수 있도록 할예정이다.
	 * this : 자기자신과 이름이 같은데 문자열을 매개변수로 가진 명령어
	 * 
	 * 
	 * 생성자에서는 new StudentServiceImplV1("파일명")의 
	 * 생성자 메서드를 다시 호출하는 구조가된다.
	 * 
	 * 
	 */
	...
	public void printStudent() {
		 ...
		 * i<배열.length
		 *  List객체는 특성상 수시로 데이터의 갯수가 변경된다
		 *  List에 담긴 갯수를 알려주는 size() method는 호출 될때마다 List에 담긴 데이터 갯수를 다시 헤아리는 코드가 실행된다
		 *  for ()반복문 내에서 i< List.size() 코드를 사용하면 for 문이 반복되는동안 계속해서 갯수를 헤아리게 된다
		 *  데이터의 갯수가 몇개 안될때는 큰문제가 없지만
		 *  데이터 갯수가 많아지만 그만큼 성능이 떨어진다
		 *  
