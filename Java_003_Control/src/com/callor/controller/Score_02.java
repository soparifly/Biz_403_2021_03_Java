...
public class Score_02 {
	...
	public static void main(String[] args) {
		...
		// 문법 오류로 보여준다
		// 개발자가 실제 값의 소수점 이하가 잘리는것을 알고있으며 소수점 이하의 값이 필요없어서 정수형 변수에 저장하고싶다 이럴때는 컴파일러에게
		// 강제로 알려준다

		intNum1 = (int) 1.0F; // 오류가 없다는 표시로 명령문 (int)를 입력해준다
		intNum1 = (int) floatNum11;

		/*
		 * 자동형 변환 (암시적,묵시적 형변환)
		 * (정수형=>실수형) 정수값, 정수형 변수에 저장된 값을 실수(float, double)형 변수에 저장하려고 할때 정수값.0
		 * 형식의 실수로 자동으로 변환한 다음 저장을 한다. 매우 큰값은 오차라 발생할수 있지만 일반적으로 사용하는 숫자는 오차없이 실수형 변수에
		 * 저장이 가능하다.
		 * 
		 */
		floatNum11 = intNum1;
		 ...
		 * 강제형 변환 (명시적 형변환)ㄴ
		 * 
		 * 실수값, 실수형 변수에 저장된 값을 정수형 변수에 저장하려고 시도하면 컴파일러는 이코드는 문제를 일으킬수 있다는 것을 인식하고 오류를
		 * 보여준다. *
		 * 만약 작석자가 오차를 무시하고 강제로 실수값을 정수형에 변환하여 저장하고자 한다면 
		 * "강제로 변환하는" 코드를 첨가하면된다
		 * (int)
		 */
		intNum1 = (int)floatNum11;
