...
public class Score_03 {
	public static void main(String[] args) {
		 ...
		 * 하지만, 여전히 계산된 결과값은 소수점 이하 값이 0으로 나타나고있다
		 * 
		 * 이상황은 대입 연산자의 성질때문이다.
		 * 대입 연산자가 나오면 항상 오른쪽의 연산식이 먼저 
		 * 실행된다. 결과적으로 정수 / 정수 는 정수 이며
		 * 이미 계산결과에서 소수점 이하 값이 없는 상태이다
		 * 이값을 실수형 변수에 담는 것은 아무런 의미가 없다
		 * 
		 * 이명령문을 실행하여 정확한 평균을 계산하기위해서는 
		 * 대입연산자 오른쪽의 계산식의 값들을 실수형으로 "강제형변환" 한다음 연산을 수행해야한다.
		 */
		
		//평균을 저장할 변수를 float형으로 
		float floatAvg = intSum / 2 ;
		//intSum/2의 연산식에서
		//정수형 변수 intSum을 float형으로 강제형변환하면
		//젯수(나누는수) 2.0으로 변환된다
