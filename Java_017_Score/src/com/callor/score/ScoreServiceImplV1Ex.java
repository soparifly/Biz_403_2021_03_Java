...
public class ScoreServiceImplV1Ex extends ScoreServiceImplV1 {
	...
	public ScoreServiceImplV1Ex() {
		...
		/*
		 * MenuServiceImplV1클래스의 생성자는 두개의 매개변수(아규먼트,파라메터)를 전달하면서 호출을 해야한다. 첫번째, 매개변수는
		 * 프로젝트의 title 두번째, 매개변수는 업무리스트를 담은 리스트형 객체
		 * 
		 * title 은 문자열이기때문에 바로전달할수 있는데 List형 객체는 List객체를 생성하고, 데이터를 add한후 전달해하므로 약간의 추가
		 * 연산 코드가 필요핟 이럴때는 생성자 method에서 작성하지 않고 객체를 사용하는 메소드에서 작성하는 것이 좋다*
		 */
...
//			먄약 menu변수의 값이 1일때
//			menu가 1일때 코드가 실행되고
//			코드가 종료되면 또다시  menu값이 2인가, 3인가를 검사하는 불필요한 코드가 실행된다
//			if(menu==1) {//menu가 1일때	}
//			if(menu==2) {	}
//			if(menu==3) {	}
//				2번 코드
