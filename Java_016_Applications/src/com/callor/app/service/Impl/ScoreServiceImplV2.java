...
public class ScoreServiceImplV2 implements ScoreService{
	...
	public void selectMenu() {
		...
		/*
		 * 보통 맴버변수 (클래스 영역에 선언된 변수들) 객체는 이 클래스의 생성자에서 만드는 것이 좋다
		 * 
		 * 하지만
		 * 객체를 생성할때 전달해야할 매개변수(파라미터)에 대하여 수행해야할 연산코드가 있을 경우에는 
		 * 객체를 사용하기 전에 객체를 생성하는 코드를 작성 하여도 된다
		 */
		menuService = new MenuServiceImplV1("빛 고을 고교 성적처리"
																	,scoreMenu);
		while (true) {
